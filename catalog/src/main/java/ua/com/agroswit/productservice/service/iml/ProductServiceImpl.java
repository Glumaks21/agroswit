...
public class ProductServiceImpl implements ProductService {
    ...
    private ProductState getState(Product product) {
        ...
        if (product.getCreatedAt().isAfter(sevenDaysAgo)) {
                ...
                .map(p -> {
                    for (var i : inventories) {
                        if (i.productId().equals(p.getId())) {
                            return mapper.toSimpleDetailedDTO(p, i, getState(p), uploadService.getUrl(p.getImage()));
                        }
                    }
                    return mapper.toSimpleDetailedDTO(p, getState(p), uploadService.getUrl(p.getImage()));
                })
                .toList();
    }
    ...
    public DetailedProductDTO getDetailedById(Integer id) {
        ...
        if (!inventories.isEmpty()) {
            return mapper.toDetailedDTO(product, inventories.getFirst(), getState(product), uploadService.getUrl(product.getImage()));

        }

        return mapper.toDetailedDTO(product, getState(product), uploadService.getUrl(product.getImage()));
    }
