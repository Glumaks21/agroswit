...
public class InventoryServiceImpl implements InventoryService {
    ...
    public Mono<InventoryDTO> save(InventoryDTO dto) {
        ...
        var articleCheck = inventoryRepo.existsById(dto.article1CId())
                .flatMap(e -> e
                        ? Mono.error(new ResourceInConflictStateException(String.format(
                        "Inventory record with 1c id %d already exists", dto.article1CId())))
                        : Mono.just(false)
                );
        var productCheck = inventoryRepo.existsByProductId(dto.productId())
                .flatMap(e -> e
                        ? Mono.error(new ResourceInConflictStateException(String.format(
                        "Inventory record for product id %d already exists", dto.productId())))
                        : Mono.just(false)
                );
