...
public class UploadController {
    ...
    ResponseEntity<Void> store(@RequestParam MultipartFile file) {
        ...
        return ResponseEntity.created(ServletUriComponentsBuilder.fromCurrentRequest().path("/{fileName}")
                        .buildAndExpand(fileName)
                        .toUri())
                .build();
    }

    @Operation(summary = "remove file by name")
    @DeleteMapping("/{fileName}")
    void remove(@PathVariable String fileName) {
    }
}
