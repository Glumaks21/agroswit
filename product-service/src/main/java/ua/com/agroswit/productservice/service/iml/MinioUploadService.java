...
public class MinioUploadService implements UploadService {
    ...
    public String getUrl(String name) {
        ...
        return minioProps.getEndpoint() + "/" + minioProps.getBucket() + "/" + name;
    }

    @Override
    public Resource load(String name) {
        try {
            log.info("Trying to load object {} from bucket {}", name, minioProps.getBucket());
    ...
    public String uploadImage(MultipartFile file) {
        ...
        initializeBucket();
        log.info("Trying to upload image object {} to bucket {}", file.getOriginalFilename(), minioProps.getBucket());
    ...
    private void initializeBucket() {
        try {
            log.info("Trying to initialize bucket: {}", minioProps.getBucket());
            if (!minioClient.bucketExists(BucketExistsArgs.builder()
                ...
                minioClient.setBucketPolicy(SetBucketPolicyArgs.builder()
                        .bucket(minioProps.getBucket())
                        .config(jsonAnonymousReadPolicy.replace("${bucketName}", minioProps.getBucket()))
                        .build());
            }
        } catch (GeneralSecurityException | IOException | MinioException e) {
    ...
    public void remove(String name) {
        try {
            log.info("Trying to delete objet {} from bucket {}", name, minioProps.getBucket());
            ...
            throw new UploadServiceException(String.format(
                    "Unable to delete object %s from bucket %s", name, minioProps.getBucket()), e);
        }
    }
}
